    ...
    var body: some View {
        Button(action: onSelect) {
            HStack(spacing: 10) {
                ...
                // Channel artwork — glow on active
            ...
            .background {
                ZStack(alignment: .leading) {
                    if isActive {
                        ...
                        // Neon vertical pill indicator on far left edge
                        Capsule()
                            .fill(
                                LinearGradient(
        ...
        HStack(spacing: 1.5) {
            ForEach(0..<3, id: \.self) { index in
                Capsule()
                    ...
                    .frame(width: 2, height: (animate && isVisible) ? barHeight(index) : 3)
                    .animation(
                        (animate && isVisible) ?
                            .easeInOut(duration: 0.38)
                            .repeatForever(autoreverses: true)
                            .delay(Double(index) * 0.13) : .default,
